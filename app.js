...
import { createAccountDomain } from "./account.js";
...
const projectsDomain = createProjectsDomain({
  ...
  prepDailyDefaults, backupIsDue: () => accountDomain.backupIsDue(), backupStatusLabel: () => accountDomain.backupStatusLabel(), messageCopy, CAT_COLORS
});
const {
...
accountDomain = createAccountDomain({
  getState: () => D, document, window, URL, currentSchemaVersion: CURRENT_SCHEMA_VERSION,
  appModes: APP_MODES, getCurrentUser: () => currentUser, getSyncText: () => APP_RUNTIME.syncText,
  storage, normalizeAppState, replaceState: (nextState) => { D = nextState; },
  ensureDisplayNameFromAuth, save, renderHub, renderDaily, renderAll, nav, prepJournalDefaults,
  prepDailyDefaults, toast, formatSavedAt, setText, escapeHtml, visibleProjects,
  visibleJournalEntries, visibleDailyTasks, visibleRituals, isPhoneUI, syncNotificationUI,
  refreshJournalReminderDebug, renderRitualManager
});
const {
  applyThemeSettings, setThemeMode, setThemePreset, updateCustomTheme,
  updateUserChip, updateFeatureVisibility, exportFullBackup, importFullBackup,
  renderAccount, setAppMode, setProjectLimit, toggleSetting
} = accountDomain;
